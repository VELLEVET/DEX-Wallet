  ...
  fetchData(savedState).then(items => {
    ...
    items.forEach(item => {
      ...
      arri[count].quotes.push({
        ...
        quote_volume: nFormatter(item.result.quote_volume, 2),
        latest: Number(item.result.latest).toFixed(objMap[bNum].precision),
  ...
  return Promise.all(
    ...
    .then(items => {
      for (let i = 0; i < currentRoute.quotes.length; i++) {
        ...
        routObj.quote_volume = nFormatter(items[i].result.quote_volume, 2);
